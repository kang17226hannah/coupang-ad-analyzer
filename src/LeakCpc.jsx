...
export default function LeakCpc({ rows, historyRows=[], settings, master }) {
  ...
  const productComparisons=useMemo(()=>{
    ...
    return Object.values(map).map(items=>({productId:items[0].productId,product:items[0].product,now:metrics(items,settings),before:metrics([],settings)}));
  },[rows,settings]);

  const historyMap=useMemo(()=>{
    const map={};
    for(const row of historyRows){const key=`${row.productId}|||${row.keyword}|||${row.placement}`;(map[key] ||= []).push(row)}
    return map;
  },[historyRows]);

  const groups=useMemo(()=>{
    const map={};
    for(const row of rows){const key=`${row.productId}|||${row.keyword}|||${row.placement}`;(map[key] ||= []).push(row)}
    return Object.entries(map).map(([key,items])=>{
      ...
      const noKeyword=missingKeyword(first.keyword),placement=placementCategory(first.placement);
      ...
      if(noKeyword){status={label:`${placement} · 키워드 없음`,level:'normal'};action='키워드 제외 대상 아님';rationale=`광고보고서에 검색어가 없는 ${placement} 지면 데이터입니다.`}
      else if(resolution.status==='ambiguous'){status={label:'마진 연결 필요',level:'normal'};action='옵션ID 연결';rationale='정확한 손익 기준을 계산하려면 상품 마스터 연결이 필요합니다.'}
      else if(!margin){status={label:'마진 입력 필요',level:'normal'};action='상품 마스터 입력';rationale='광고 전 개당 마진이 없어 손익 경계를 계산할 수 없습니다.'}
      else if(m.orders>0){
      ...
      }else if(cumulative.orders>0){status={label:'재관찰',level:'normal'};action='다음 보고서에서 재확인';rationale=`현재 기간 주문은 0건이지만 직전 분석기간을 포함하면 ${cumulative.orders}건의 전환이 있습니다.`}
      else if(cumulative.actualCost>=margin){status={label:'제외 검토',level:'danger'};action='입찰 축소·키워드 제외 검토';rationale=`누적 주문 0건인데 실제 광고비 ${won(cumulative.actualCost)}가 1건 마진 ${won(margin)}을 넘었습니다.`}
      else if(cumulative.actualCost>=margin*.7||remainingClicks===0){status={label:'주의',level:'warn'};action='추가 클릭 전 입찰 점검';rationale=remainingClicks===0?'현재 CPC 수준이면 추가 1클릭으로 1건 마진 경계를 넘을 수 있습니다.':`1건 마진 경계까지 ${won(Math.max(remainingSpend,0))} 남았습니다.`}
      else{status={label:'관찰',level:'normal'};action='다음 보고서에서 재확인';rationale=`1건 마진 경계까지 ${won(Math.max(remainingSpend,0))} 남아 있습니다.`}
  ...
  const visible=groups.filter(g=>{if(mode==='zero')return !g.noKeyword&&g.orders===0;if(mode==='cpc')return !g.noKeyword&&g.orders>0&&g.cpcGap>0;if(mode==='placement')return g.noKeyword;return g.noKeyword||g.status.label!=='유지'});

  return <div className="cpc-page-wrap">
    <div className="cpc-view-tabs"><button className={view==='diagnosis'?'active':''} onClick={()=>setView('diagnosis')}>광고 진단</button><button className={view==='keyword'?'active':''} onClick={()=>setView('keyword')}>키워드 누수 · CPC</button></div>
    ...
    {view==='keyword'&&<section className="panel page-panel cpc-page">
      ...
      <div className="cpc-filters"><button className={mode==='issues'?'active':''} onClick={()=>setMode('issues')}>점검 대상</button><button className={mode==='zero'?'active':''} onClick={()=>setMode('zero')}>주문 0</button><button className={mode==='cpc'?'active':''} onClick={()=>setMode('cpc')}>CPC 개선 필요</button><button className={mode==='placement'?'active':''} onClick={()=>setMode('placement')}>키워드 없음 지면</button></div>
