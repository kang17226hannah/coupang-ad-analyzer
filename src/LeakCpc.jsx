...
import { leakLevel, metrics, productContributionMargin, productMarginResolution } from './analysis';
...
export default function LeakCpc({ rows, settings, master }) {
  ...
  const groups=useMemo(()=>{
    ...
    return Object.values(map).map(items=>{
      ...
      let status;
      if(resolution.status==='ambiguous') status={label:'마진 연결 필요',level:'normal'};
      else if(m.orders===0) status=leakLevel(m,settings);
      ...
      return {productId:first.productId,product:first.product,keyword:first.keyword,...m,margin,resolution,breakEvenCpc,cpcGap,status};
    }).sort((a,b)=>b.actualCost-a.actualCost);
  },[rows,settings,master]);

  const visible=groups.filter(g=>{
    const zeroLeak=g.orders===0&&leakLevel(g,settings).rank>0;
    const missingMargin=g.orders>0&&(!g.margin||g.resolution.status==='ambiguous');
  ...
  return <div className="cpc-page-wrap">
    ...
    {view==='keyword'&&<section className="panel page-panel cpc-page">
      <div className="panel-title"><div><h2>키워드 누수 · CPC 분석</h2><p>실제 CPC는 광고비에 VAT 10%를 포함합니다. 손익분기 CPC = 광고 전 개당 마진 × CVR.</p></div></div>
      ...
      <div className="table-wrap wide"><table><thead><tr><th>상품 / 키워드</th><th>상태</th><th>클릭</th><th>주문</th><th>실제 광고비</th><th>실제 CPC</th><th>CVR</th><th>손익분기 CPC</th><th>CPC 여유</th><th>실제 ROAS</th></tr></thead><tbody>
        {visible.map(g=><tr key={`${g.productId}-${g.keyword}`}><td><b>{g.keyword}</b><small className="block">{g.product} · {g.productId}</small></td><td>{badge(g.status)}</td><td>{g.clicks}</td><td>{g.orders}</td><td>{won(g.actualCost)}</td><td>{won(g.cpc)}</td><td>{pct(g.cvr)}</td><td>{g.breakEvenCpc===null?(g.resolution.status==='ambiguous'?'마진 연결 필요':g.orders>0?'마진 입력 필요':'—'):won(g.breakEvenCpc)}</td><td className={g.cpcGap!==null?(g.cpcGap>=0?'good-text':'bad-text'):''}>{g.cpcGap===null?'—':`${g.cpcGap>=0?'+':''}${won(g.cpcGap)}`}</td><td>{pct(g.actualRoa)}</td></tr>)}
      </tbody></table></div>
